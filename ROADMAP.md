...
- [ ] **CEPAL — geoportal del Censo Ecuador (`geo.cepal.org/censo-ecuador/`), aportado por Daniel 2026-08-30.** Es una app MapLibre, pero su config (`maps/geoportal/config.js`) declara dos APIs REST reales detrás: `geodata.cepal.org/api/v1` (datos estadísticos amarrados a las geometrías `cantones_del_ecuador0`/`provincias_ecuador0` — devolvió 502 en la prueba en vivo, puede ser intermitente) y `geoportal.cepal.org` (instancia **GeoNode** pública y funcionando, confirmado `/api/v2/datasets/` con 881 recursos totales en la plataforma). El filtro por palabra clave (`geonode_keyword = 'geoportal_inec'`) que debería acotar a solo capas de Ecuador no funcionó con el parámetro que probé (`keywords__slug`) — el primer resultado fue una capa global de áreas protegidas, no algo específico de Ecuador. Encaja con el ítem ya existente de "Fuentes internacionales" (CEPAL como primer candidato) — esto es la entrada concreta a esa pieza, con la salvedad de que hace falta mapear bien el filtro/categoría correcta antes de integrar nada. → conversación 2026-08-30
- [ ] **INEC — micrositio propio de Geografía Estadística con el Clasificador Geográfico oficial, año por año (incluye 2026), aportado por Daniel 2026-08-30.** `ecuadorencifras.gob.ec/documentos/web-inec/Geografia_Estadistica/Micrositio_geoportal/` archiva shapefiles + listados de códigos DPA para 2001, 2005-2026 completos, más geodatabases nacionales y cartografía histórica (mapas censales de 1974 y 1990 por provincia). **Esta es la fuente oficial que debería respaldar `helpers/geo_data.py`, pero no lo hace hoy** — `scripts/fetch_cantones.py` construyó el JSON offline actual desde un FeatureServer de ArcGIS de terceros (`services7.arcgis.com/.../Cantones_del_Ecuador`), de vigencia no documentada en el código, no desde este clasificador oficial. Acción concreta: comparar el `CLASIFICADOR_GEOGRAFICO_2026.zip` oficial contra `helpers/data/cantones.json`/`provincias.json`/`parroquias.json` actuales para confirmar si hay drift real (cambios de código, cantones nuevos, fusiones) antes de decidir si hace falta reconstruir la referencia offline desde la fuente oficial. → conversación 2026-08-30
