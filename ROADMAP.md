...
- [x] Sector eléctrico — **`sisdatbi.arconel.gob.ec` (SISDAT BI), revisitado y descartado definitivamente 2026-09-04**. Desde la red normal el host da timeout TCP puro (sin respuesta en el puerto 80 ni 443) — parecía caído, pero con VPN activada responde de inmediato: es un bloqueo geográfico/por IP, no una caída real del servidor. Con VPN, HTTP redirige (301) a HTTPS y HTTPS sirve una página de login real (`<title>SISDAT - BI</title>`, `<body id="login">`, plantilla AdminLTE) con un único campo `<input type="password" name="txtupass">` — sin campo de usuario, sin rol de invitado, sin iframe/embed público visible en el HTML servido. No es un dashboard tipo Power BI con `reportEmbed`; es una aplicación PHP propia con login obligatorio. Descartado como fuente: no hay contenido accesible sin credenciales. → RESEARCH.md § Duodécima pasada
- [x] Sector eléctrico — **CELEC EP: reportes propios de transparencia/rendición de cuentas, evaluado y descartado 2026-09-04**. Páginas genéricas de transparencia corporativa son shells vacíos; las páginas sector-específicas ("Balance Energético Nacional", "Plan Maestro de Electricidad") parecen duplicar documentos ya en CENACE Biblioteca. El contenido real está en LOTAIP por unidad de negocio (13 unidades, ej. celecsur/lotaip/transparencia-2023/ con 227 links reales) pero es cumplimiento administrativo genérico (organigramas, salarios, contratos, auditorías) igual al de cualquier institución pública, no dato del sector eléctrico — no se recomienda construir. → RESEARCH.md § Duodécima pasada
