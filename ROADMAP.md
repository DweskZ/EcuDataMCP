...
- [ ] **`search_inec_noticias` — nuevo tool: descubrir boletines vigentes vía la sección de Noticias del INEC, no solo las páginas estáticas de tema.** Auditadas en vivo las 74 páginas de tema que indexa `search_inec_estadisticas` (2026-08-29/30, pedido de Daniel con VPN a LatAm; ver ENEMDU abajo para el caso que arrancó esto): **26 de 74 no tienen ningún archivo propio** — son páginas "hub" que solo repiten el menú completo del sitio (confirmado leyendo el HTML crudo de varias), no páginas de contenido con su propio listado. De las 48 restantes con archivos, varias están abandonadas hace años (Alquileres 2013, Educación/Asentamientos humanos 2017, Protección social 2018) mientras el sitio en general sigue vivo y publicando (IPC e INPP de julio 2026, ENIGHUR 2024-2025, empleo de mayo 2026 — todo confirmado real). El patrón es el mismo en cada caso: INEC no dejó de publicar, dejó de enlazar el boletín nuevo desde la página de tema vieja, y ahora solo lo anuncia en `/institucional/noticias/` (paginado, ~43 links por página, confirmado con boletines reales de IPC/INPP/ENIGHUR/empleo, todos con PDF/XLSX adjuntos reales bajo el mismo patrón de ruta `documentos/web-inec/{TEMA}/{año}/{Mes}_{año}/...`). Ojo: `/institucional/boletines/` existe pero es la página de prensa del Censo 2022, no un índice general — no sirve para esto. Acción: nuevo tool que scrapea Noticias por palabra clave/tema y devuelve el boletín vigente + sus adjuntos, usable como fallback cuando `get_inec_estadistica_files` devuelve una página de tema vacía o visiblemente vieja. → RESEARCH.md § Novena pasada
- [ ] **Las 4 categorías "macro" del menú de INEC (Estadísticas Macroeconómicas, Cuentas económicas, Comercio internacional y balanza de pagos, Finanzas públicas/fiscales) están vacías por diseño, no por abandono — verificar que de verdad no publican nada propio antes de asumir que son solo otro caso del ítem de arriba.** Las 4 son puro "hub" (0 archivos, HTML idéntico al menú repetido). Hipótesis a confirmar: Cuentas Nacionales y Balanza de Pagos son responsabilidad legal del BCE en el sistema estadístico ecuatoriano (ya cubierto por `search_indicadores_bce`/IEM), así que INEC podría no publicar nada propio ahí — a diferencia de Empleo/Precios, donde si hay contenido real solo que mal enlazado. Confirmar contra Noticias antes de descartar. → RESEARCH.md § Novena pasada
