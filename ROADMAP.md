...
- [ ] **INEC: no hay preview de archivos grandes (ZIP/BDD), a diferencia de CKAN.** `get_inec_publicacion_archivos`/`get_topic_files` solo devuelven metadata (label/url/formato) — nunca descargan el archivo, por diseño (igual que SIPA). Pero a diferencia de los recursos CKAN, que tienen `preview_zip`/`preview_targz` para ver una muestra de filas sin bajar todo, **no existe ningún tool que muestre una vista previa de un ZIP/BDD de INEC** (ej. el REESS `200901_202412_REESS_MENSUAL_BDD_DEFINITIVAS.zip`, confirmado en vivo: no terminó de bajar los primeros 100 MB en 60s, claramente muy por encima del cap de 5 MB). Un agente que reciba solo `{label, url, formato}` no tiene forma de inspeccionar el contenido sin salir del MCP. Además, `read_pdf` no valida la extensión/content-type antes de descargar — si alguien lo apunta por error a uno de estos ZIP, baja hasta 5 MB inútilmente antes de fallar con "no es un PDF válido". Acción: extender `preview_zip`/`preview_targz` (o un wrapper específico para INEC) para que acepten una URL directa, no solo un `resource_id` de CKAN; validar extensión/content-type en `read_pdf` antes de descargar. → conversación 2026-08-30
