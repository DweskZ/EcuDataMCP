...
- [ ] **Reemplazar el descubrimiento de contenido de `inec_client.py` por la API REST pública de WordPress (`/wp-json/wp/v2/posts`), no scraping de HTML.** Corrección importante 2026-08-30 (Daniel aportó URLs reales que refutaron el diagnóstico anterior de este mismo roadmap): `search_inec_estadisticas`/`get_inec_estadistica_files` derivan su lista de 74 "temas" del menú (mega-menu) de una sola página semilla (`indice-de-precios-al-consumidor/`) — pero **el menú no es el mismo en cada página del sitio**. La página `estadisticas-laborales-enemdu/` (no está en nuestra lista de 74) tiene su propio menú de ~109 links que incluye `enemdu-anual/`, `enemdu-trimestral/`, `enemdu-telefonica/`, `empleo-y-condicion-de-actividad/`, `trabajo-y-empleo/`, `matrices-de-transicion-laboral/` — ninguno alcanzable desde la página semilla actual. Verificado en vivo: `enemdu-anual/` tiene el ENEMDU anual 2025 completo (BDD SPSS/CSV, boletín técnico, tabulados), `enemdu-trimestral/` tiene hasta 2026-I, `estadisticas-laborales-enemdu/` tiene 59 archivos mensuales hasta mayo 2026 — es decir, **ENEMDU 2024-2026 sí está publicado en el sitio principal de INEC**, el gap real era de descubrimiento, no de publicación (corrige el ítem anterior de este roadmap, que asumía que solo aparecía vía Noticias). No hay `sitemap.xml`/`wp-sitemap.xml` en el dominio, pero sí existe `/wp-json/wp/v2/posts` (API REST de WordPress, pública, sin auth): 1,707 posts totales, el más nuevo del 2026-08-25, con `search`, `orderby=date`, paginación y el HTML de cada post (`content.rendered`) trayendo los enlaces a archivos directamente — confirmado extrayendo 11 archivos reales del post `enemdu-anual-2024` (`/wp-json/wp/v2/posts?slug=enemdu-anual-2024`) sin tocar ninguna página HTML. Esto es la fuente de verdad real: Noticias y Boletines (`/institucional/...`) son solo vistas filtradas por categoría de esta misma colección de posts, no una fuente aparte — no hace falta un scraper de Noticias distinto, hace falta consumir la API REST directamente. Acción: reescribir el descubrimiento de `inec_client.py` para buscar/paginar `/wp-json/wp/v2/posts?search=...` (con fecha `modified` real, no inferida de nombres de archivo) en vez de derivar temas del menú de una página fija; mantener las páginas de tema viejas solo como fallback si la API deja de responder. → RESEARCH.md § Novena pasada
- [ ] **Las 4 categorías "macro" del menú de INEC (Estadísticas Macroeconómicas, Cuentas económicas, Comercio internacional y balanza de pagos, Finanzas públicas/fiscales) siguen sin archivos propios, pero dado el hallazgo de arriba esto ya no se puede asumir sin más — confirmar contra la API de posts antes de descartar.** Las 4 son puro "hub" en HTML (0 archivos, menú repetido). Hipótesis a confirmar: Cuentas Nacionales y Balanza de Pagos son responsabilidad legal del BCE en el sistema estadístico ecuatoriano (ya cubierto por `search_indicadores_bce`/IEM), así que INEC podría no publicar nada propio ahí — pero el caso de ENEMDU (arriba) ya demostró que "0 archivos en la página hub" no implica "sin contenido real en el sitio". Buscar en `/wp-json/wp/v2/posts?search=` con los términos de estas 4 categorías antes de concluir que de verdad no hay nada. → RESEARCH.md § Novena pasada
