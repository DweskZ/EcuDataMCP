...
| Contrato de respuesta para agentes | Parcial — falta extender a otras fuentes | Las 119 tools ya devuelven `structuredContent`, pero el bloque `metadatos` estable solo cubre 4 (BCEData/IEM y las 2 de mantenimiento) y el `outputSchema` publicado es genérico en todas — ver MCP_ARCHITECTURE.md § Fases 5 y 6 para el plan medido; falta además paginar/enlazar archivos grandes en vez de enviarlos completos |
| Operación 24/7 | Parcial — falta alertas de esquema | Smoke test diario (`scripts/smoke_e2e.py`) corre en GitHub Actions, separado de CI, pero llama 43 de las 119 tools (36%): 76 nunca se ejercitan contra la fuente real; falta alertar sobre cambios de esquema específicos → RESEARCH.md § Infraestructura operativa |
...
| Sector eléctrico — dominio profundo (CENACE/ARCONEL/CNEL) | Parcial — `reportes.arconel.gob.ec` construido (ver Hecho) | Datasets CKAN de CENACE/CNEL EP/ARCONEL/IIGE y los reportes de ARCONEL ya cubiertos; falta CENACE Biblioteca (documentos de planificación: Plan Maestro de Electricidad, etc.) sin tocar; EEQ/Centrosur/EERSA/EEASA sin organización CKAN propia → RESEARCH.md § Vigésima y Trigésimo segunda pasada |
