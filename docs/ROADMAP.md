...
| Centrosur — cortes de luz programados (incluida la crisis 2024) | `search_centrosur_cortes` | Archivo de PDFs de "Programación de cortes del servicio de energía eléctrica" para Azuay/Cañar/Morona Santiago, 2023-presente, incluida la crisis por estiaje sep-dic 2024; enumerado vía la API REST de Media Library de WordPress (`wp-json/wp/v2/media?search=...`), sin login — el único distribuidor del proyecto con un mecanismo de enumeración programática real para este tipo de documento. No cubre la carpeta separada `CortesEstiaje/` (subida fuera de la media library, sin mecanismo de enumeración conocido) → RESEARCH.md § Vigésimo séptima pasada |
...
| Contrato de respuesta para agentes | Parcial — falta extender a otras fuentes | Las 117 tools ya devuelven `structuredContent`, pero el bloque `metadatos` estable solo cubre 4 (BCEData/IEM y las 2 de mantenimiento) y el `outputSchema` publicado es genérico en todas — ver MCP_ARCHITECTURE.md § Fases 5 y 6 para el plan medido; falta además paginar/enlazar archivos grandes en vez de enviarlos completos |
| Operación 24/7 | Parcial — falta alertas de esquema | Smoke test diario (`scripts/smoke_e2e.py`) corre en GitHub Actions, separado de CI, pero llama 43 de las 117 tools (37%): 74 nunca se ejercitan contra la fuente real; falta alertar sobre cambios de esquema específicos → RESEARCH.md § Infraestructura operativa |
...
| Sector eléctrico — dominio profundo (CENACE/ARCONEL/CNEL) | No iniciado, protocolo mapeado a fondo | Datasets CKAN de CENACE/CNEL EP/ARCONEL/IIGE ya cubiertos (ver Hecho); falta `reportes.arconel.gob.ec` (ASP.NET ReportViewer, sin login, 1998-2026), mapeado byte a byte en la Vigésima pasada (2026-09-06) — secuencia exacta de postbacks, formato delta de MS AJAX, y algoritmo para ubicar la grilla real bajo envoltorios anidados, todo sin `BeautifulSoup`/`lxml`; criterio de fin de paginación confirmado en la Trigésimo segunda pasada (2026-09-25: `TotalPages` sin `?` y igual a `CurrentPage`; el "Siguiente" real es `__doPostBack` sobre el div `Next$ctl00`) — **listo para escribir el cliente**; CENACE Biblioteca (documentos de planificación) sin tocar; EEQ/Centrosur/EERSA/EEASA sin organización CKAN propia → RESEARCH.md § Vigésima y Trigésimo segunda pasada |
| Archivo histórico de cortes de luz (crisis sep-dic 2024) — resto de distribuidoras | Parcial — Centrosur, EEQ y el snapshot de energia-ecuador.com ya construidos (ver Hecho) | EEQ: la lista semilla puede tener huecos (sep-oct 2024 y abril 2024 solo por buscador) — ampliar con más búsquedas dirigidas; los PDFs aún no se parsean a filas (sector × horario). Centrosur: carpeta `CortesEstiaje/` sin enumerar. EEASA con URLs reales conocidas en su propio sitio pero dominio caído al momento de esta pasada, reintentar más adelante; CNEL confirmado sin archivo recuperable por ninguna vía probada (ni su WordPress por tag/búsqueda, ni Wayback Machine — sus páginas en energia-ecuador.com nunca tuvieron una captura 200); EERSSA bloqueada por 403 de servidor confirmado con tres métodos distintos → RESEARCH.md § Vigésimo séptima, Vigésimo octava y Trigésimo primera pasada |
...
| Presupuesto de contexto de `tools/list` | No iniciado | `tools/list` pesa 188.352 caracteres (≈47k tokens, ~24% del contexto de un cliente de 200k) antes de la primera pregunta; mediana 1.616 por tool. Falta bajar el detalle largo a `docs/TOOLS.md`/`ecuador://fuentes`, extender `MCP_PROFILE` a toolsets por dominio y poner un gate de tamaño en CI → MCP_ARCHITECTURE.md § Fase 4 |
...
| Telemetría de uso de tools | No iniciado | Nada registra llamadas, latencia ni tasa de error por tool, y toda la revisión de arquitectura condiciona retirar tools a "medir llamadas reales" → MCP_ARCHITECTURE.md § Fase 8 |
