...
| Contrato de respuesta para agentes | Parcial — falta extender a otras fuentes | Las 120 tools ya devuelven `structuredContent`, pero el bloque `metadatos` estable solo cubre 4 (BCEData/IEM y las 2 de mantenimiento) y el `outputSchema` publicado es genérico en todas — ver MCP_ARCHITECTURE.md § Fases 5 y 6 para el plan medido; falta además paginar/enlazar archivos grandes en vez de enviarlos completos |
| Operación 24/7 | Parcial — falta alertas de esquema | Smoke test diario (`scripts/smoke_e2e.py`) corre en GitHub Actions, separado de CI, pero llama 43 de las 120 tools (36%): 77 nunca se ejercitan contra la fuente real; falta alertar sobre cambios de esquema específicos → RESEARCH.md § Infraestructura operativa |
...
| Archivo histórico de cortes de luz (crisis sep-dic 2024) — resto de distribuidoras | Parcial — Centrosur, EEQ y el snapshot de energia-ecuador.com ya construidos (ver Hecho) | EEQ: la lista semilla puede tener huecos (sep-oct 2024 y abril 2024 solo por buscador) — ampliar con más búsquedas dirigidas; los PDFs de EEQ ya se parsean a filas (ver Hecho); faltan los de Centrosur. Centrosur: carpeta `CortesEstiaje/` sin enumerar. EEASA con URLs reales conocidas en su propio sitio pero dominio caído al momento de esta pasada, reintentar más adelante; CNEL confirmado sin archivo recuperable por ninguna vía probada (ni su WordPress por tag/búsqueda, ni Wayback Machine — sus páginas en energia-ecuador.com nunca tuvieron una captura 200); EERSSA bloqueada por 403 de servidor confirmado con tres métodos distintos → RESEARCH.md § Vigésimo séptima, Vigésimo octava y Trigésimo primera pasada |
