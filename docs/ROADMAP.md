...
| Fuentes internacionales con foco Ecuador | No iniciado, APIs concretas confirmadas | CEPALSTAT tiene un Open Data API documentado (`estadisticas.cepal.org/cepalstat/web_cepalstat/openDataAPI.asp`); el FMI tiene una API SDMX 2.1/3.0 moderna y bien documentada (`data.imf.org`, Swagger) cubriendo IFS (194 países); integrar solo lo que aporte frecuencia/corte que la fuente ecuatoriana no publique → RESEARCH.md § Fuentes externas de sociedad civil, § Vigésimo cuarta pasada |
