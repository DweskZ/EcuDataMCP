...
| Ministerio de Salud Pública | No iniciado | Dominio vivo con contenido real y sección LOTAIP; solo 8 paquetes en su organización CKAN — sub-objetivos concretos encontrados sin explorar: Gacetas Epidemiológicas (`salud.gob.ec/.../gaceta-epidemiologica/`, archivo 2012-2026), plataforma de datos COVID-19, GeoSalud en Cifras → RESEARCH.md § Séptima pasada, § Vigésimo tercera pasada |
| Homicidios Intencionales (Ministerio del Interior) | Ya reachable, sin código nuevo | Verificado en vivo con `search_datasets`: 4 archivos XLSX (mensual 2026, histórico 2014-2025, microdatos 2014-2024 de 93 MB), actualizado 2026-08-19. Solo falta documentarlo como fuente conocida — cero código nuevo necesario → RESEARCH.md § Vigésimo tercera pasada |
| Autoridad Portuaria de Puerto Bolívar (APPB) | Ya reachable, sin código nuevo | 246 paquetes CKAN — el conteo más alto de cualquier organización del portal. Reportes mensuales reales de tráfico de buques, carga import/export por producto con tonelaje y país de origen/destino, obras de inversión — actualizado hasta agosto 2026. Comercio exterior/logística portuaria que ninguna otra fuente del proyecto cubre; ya reachable vía tools CKAN genéricos → RESEARCH.md § Vigésimo tercera pasada |
| Cancillería (Ministerio de Relaciones Exteriores y Movilidad Humana) | Ya reachable, sin código nuevo | Verificado en vivo con `search_organizations`: organización CKAN con 13 paquetes reales (Apostillas y Legalizaciones, Visas, estadísticas de movilidad humana anonimizadas) — ya reachable con las tools CKAN genéricas, no hace falta ningún cliente dedicado → RESEARCH.md § Vigésimo cuarta pasada |
...
| CKAN nacional — organizaciones con muchos paquetes nunca buscadas | No iniciado | Conteo de paquetes vía `search_organizations`, contenido no verificado archivo por archivo: SRI genérico (127, separado de `search_sri_datasets`), MEF genérico (97, separado de `search_mef_fiscal`), IEPS (106), COSEDE (88, estabilidad financiera), IPAIP (70, pesca/acuicultura), MAG genérico (69, separado de SIPA). El barrido de organizaciones no se confirmó exhaustivo (`posiblemente_hay_mas: true` en la página 100) → RESEARCH.md § Vigésimo tercera pasada |
...
| Fuentes internacionales con foco Ecuador | No iniciado, 3 candidatos verificados en vivo y listos para construir | **CEPALSTAT**: API REST pública real (`api-cepalstat.cepal.org`, OpenAPI 3), sin autenticación, probada en vivo contra un indicador real (población 1950-2100) — devuelve datos/metadata/dimensiones/fuentes, con `format=json\|xml\|csv\|excel` y un parámetro `members` para acotar por país; necesita un cliente dedicado (no es CKAN). **IADB Latin Macro Watch**: 665 recursos CSV (desempleo, IPC, tipo de cambio, precios de comercio exterior, ingresos/gastos/balance fiscal por nivel de gobierno, LCU y USD, mensual/trimestral/anual desde 1990, 26 países incluido Ecuador) — confirmado que vive en `data.iadb.org`, un portal **CKAN estándar** (mismo API `/api/3/action/package_search` que el portal nacional). **World Bank/IADB Database of Political Institutions (DPI)**: mismo portal CKAN, un CSV con ~180 países 1975-2023 (fechas electorales, pesos y contrapesos, fragmentación partidaria) + codebook PDF. El FMI también tiene una API SDMX 2.1/3.0 (`data.imf.org`) cubriendo IFS (194 países), sin verificar en vivo todavía. Los dos de IADB podrían añadirse como un nuevo `source` al cliente CKAN genérico ya existente (mismo patrón que `cuenca`/`latacunga`), sin scraper nuevo; CEPALSTAT sí necesita un cliente propio → RESEARCH.md § Fuentes externas de sociedad civil, § Vigésimo cuarta pasada, § Vigésimo quinta pasada |

### Otros ítems parciales

| Fuente | Estado | Qué falta |
|---|---|---|
| SENESCYT/Educación Superior | Parcial | Cubierto vía CKAN + biblioteca de Educación Superior; registro de títulos bloqueado por captcha (no automatizable). Encontrado sin explorar: `siau.senescyt.gob.ec/estadisticas-de-educacion-superior-ciencia-tecnologia-e-innovacion/` con fichas metodológicas, reportes de indicadores 2021/2022/2024, índice de competitividad, indicadores de ciencia/tecnología/innovación, análisis de demanda laboral y estudio de impacto COVID-19 → RESEARCH.md § SENESCYT, § Vigésimo cuarta pasada |
