...
from helpers import ckan_client, gobec_client, sercop_client
...
def register_search_ecuador_tool(mcp: FastMCP) -> None:
    @mcp.tool()
    @log_tool
    async def search_ecuador(query: str, limit: int = 5) -> str:
        """
        Unified search across Ecuador open data, trámites, regulations and contracts.
        ...
        if not query:
            return "Error: proporciona un query (ej. 'salud', 'pasaporte', 'INEC')."
        ...
        datasets_r, orgs_r, tramites_r, regs_r, contratos_r = await asyncio.gather(
            _datasets(), _orgs(), _tramites(), _regulaciones(), _contratos()
        )
        ...
        if isinstance(contratos_r, Exception):
            ...
            if not data:
                parts.append("Sin resultados este año. Prueba search_contratos con otro year.")
            else:
                parts.append(f"Encontrados: {total} (mostrando {min(len(data), limit)})")
