...
def register_get_seps_seccion_archivos_tool(mcp: MCPServer) -> None:
    @mcp.tool(title="Ver archivos de una sección de SEPS", annotations=READ_ONLY)
    @log_tool
    async def get_seps_seccion_archivos(
        ...
        try:
            ...
            raise ToolError(f"Error al obtener la sección SEPS: {e}") from e
