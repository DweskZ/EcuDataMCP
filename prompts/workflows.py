...
def register_workflow_prompts(mcp: FastMCP) -> None:
    ...
    def buscar_inec(tema: str = "empleo") -> str:
        ...
        return (
            ...
            "coincida.\n"
            f"3) **BIINEC (search_biinec_extras('{tema}'))** — solo si los pasos 1 y "
            "2 no encontraron nada. Este tool NO busca en vivo dentro de BIINEC (su "
            "flujo requiere sesión JSF, no vale la pena automatizarlo) — es una lista "
            "pequeña y verificada a mano de los pocos registros confirmados como "
            "exclusivos de BIINEC (módulos ambientales/administrativos). Si tampoco "
